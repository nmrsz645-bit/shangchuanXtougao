...
- 另以“无历史聊天的新开发电脑”方式新建临时 Git 克隆、全新 `.venv` 并严格按本文档安装依赖后复验同样的 145 项测试和 `compileall`；主界面 `投稿中心.py` 启动 3 秒持续存活。首次启动仅生成本机 `个人数据`、`共享飞书设置.json`、`API投稿2.0\config/data/logs` 等默认运行目录，它们都处于 Git 忽略范围，未写入源码仓库。
...
- 新电脑源码初始化已补齐 `requirements-dev.txt` 与虚拟环境命令；实际视频上传必须安装正式版 Google Chrome，不能把 Playwright 自带 Chromium 当作替代品。GitHub Actions 的 `.github/workflows/offline-tests.yml` 会在每次推送和拉取请求时自动运行离线测试与语法检查，最近一次已在 Ubuntu 与 Windows 均通过。
...
| API 跨平台离线测试入口 | `run_api_tests.py` |
